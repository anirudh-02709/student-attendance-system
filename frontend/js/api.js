  ...
  try {
    ...
    const presentToday = Array.isArray(attendanceRecords)
      ? attendanceRecords.filter((record) => {
          const recordDate = getDateKey(record.date) || getDateKey(record.markedAt);
...
async function loadDashboard() {
  ...
  const totalStudentsElement = document.querySelector("[data-total-students]");
  const presentTodayElement = document.querySelector("[data-present-today]");
  const attendancePercentElement = document.querySelector("[data-attendance-percent]");
  ...
  if (attendancePercentElement) {
    attendancePercentElement.textContent = `${data.attendancePercentage ?? 0}%`;
  }
}
